...
public class Q10162 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int fiveMinuteTimer = second / TIMER[0];
        second = second % TIMER[0];

        int minuteTimer = second / TIMER[1];
        second = second % TIMER[1];

        int tenSecondTimer = second / TIMER[2];
        second = second % TIMER[2];

        if(second != 0) {
            System.out.println(-1);
            return;
        }
        System.out.println(String.format("%d %d %d", fiveMinuteTimer, minuteTimer, tenSecondTimer));
    }
}
